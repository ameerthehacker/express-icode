...
Compiler.getCompileCmd = (
  ...
  compileCmd = compileCmd.replace(
    /:destination/g,
    path.join("home", path.basename(filename))
  );
...
Compiler.startContainer = (compiler, callback) => {
  ...
  const compileDirectory = path.join(
    appRootPath.path,
    "tmp",
    uniqid("compile-")
  );
  ...
  docker
    .run(
      ...
      null,
      {
        ...
        Volumes: { "/volume": {} },
        HostConfig: {
          Binds: [compileDirectory + ":/volume:rw"],
          Memory: memoryLimit
        }
      },
      (err, data) => {}
...
Compiler.compileCode = (
  ...
  const codeFilename = path.join(
    compileDirectory,
    Compiler.getFullFilename(compiler, "solution")
  );
  const errorFilename = uniqid("error-") + ".compile";
  let compileCmd = Compiler.getCompileCmd(
    compiler,
    Compiler.getVolumeFileName("solution"),
    Compiler.getVolumeFileName(errorFilename)
  );
  ...
  Compiler.execContainer(container, compileCmd, () => {
    const errors = fs
      .readFileSync(path.join(compileDirectory, errorFilename))
      .toString();
    if (errors.length == 0) {
...
Compiler.runCode = (
  ...
  if (compiler.run.length == 0) {
    ...
    runCmd = Compiler.getCompileCmd(
      compiler,
      Compiler.getVolumeFileName("solution"),
      Compiler.getVolumeFileName(errorFilename),
      Compiler.getVolumeFileName(inputFilename),
      Compiler.getVolumeFileName(outputFilename)
    );
  } else {
    runCmd = Compiler.getRunCmd(
      compiler,
      Compiler.getVolumeFileName("solution"),
      Compiler.getVolumeFileName(inputFilename),
      Compiler.getVolumeFileName(outputFilename)
  ...
  Compiler.execContainer(container, runCmd, () => {
    if (!resultSent) {
      const output = fs
        .readFileSync(path.join(compileDirectory, outputFilename))
        .toString();
